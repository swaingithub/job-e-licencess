...
<div class="modal fade" id="filtersModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div
        ...
        <div class="modal-content tw-rounded-none tw-relative tw-max-h-screen">
                            ...
                            @if (request('job_type'))
                                <span
                                    class="tw-py-1 tw-pl-3 tw-pr-[28px] tw-bg-[#F1F2F4] tw-text-sm tw-text-[#474C54] tw-relative tw-rounded-[30px]">{{ request('job_type') }}<span
                                        class="tw-absolute tw-right-[5px] tw-top-[3px] cursor-pointer"
                                        onclick="jobTypeClose()">
                                        <x-svg.tw-close-icon />
                                    </span>
                                </span>
                            @endif
            ...
            <div class="tw-px-5 tw-pt-5">
                ...
                <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                    <input {{ Route::current()->parameter('category') ? '' : 'checked' }} type="radio"
                        id="allcategory" class=" category-radio tw-scale-125" aria-data-id="category" value="">
                    <label for="allcategory"
                        class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">{{ __('all_category') }}</label>
                </div>
                @foreach ($categories as $category)
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ $category->slug == Route::current()->parameter('category') ? 'checked' : '' }}
                            type="radio" aria-data-id="category" data-id="{{ $category->slug }}"
                            id="{{ $category->name }}_{{ $category->id }}" class=" category-radio tw-scale-125"
                            value="{{ $category->slug }}">
                        <label for="{{ $category->name }}_{{ $category->id }}"
                            class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">{{ $category->name }}
                        </label>
                    </div>
                @endforeach
            </div>
            ...
            <div class="tw-p-5">
                ...
                <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                    <input {{ request('price_min') == 100 && request('price_max') == 1000 ? 'checked' : '' }}
                        ...
                        name="salleryRange">
                    <label for="100" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">{{ $currency_symbol }}100
                        -
            ...
            <div class="tw-absolute tw-bottom-0 tw-left-0 tw-right-0 tw-p-5 tw-bg-white tw-z-50 tw-flex tw-justify-between tw-items-center tw-mt-3">
